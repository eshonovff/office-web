...
- Молия, маош, HR
- Telegram
- Multi-tenant / `centerId` — кормандон **як** ширкатанд. Мизоҷон (`(account)/*`) танҳо
  маълумоти худро мебинанд — ҷудокунӣ дар backend (ниг. `office-api` фазаи 14)
...
9. Пеш аз истифодаи ягон калиди нав дар `t('a.b.c', { ns })`, дар
   `public/locales/ru/<ns>.json` ВА `public/locales/tg/<ns>.json`
   бо роҳи ПУРРА (масалан `actions.delete`, на танҳо `delete`) тафтиш
   кун, ки калид дар ҳамон ҷо, бо ҳамон nesting, воқеан вуҷуд дорад.
   Калиди шабеҳ дар namespace/шохаи дигар (масалан `fileInput.delete`
   ба ҷои `actions.delete`) намесозад — агар набошад, ба ҲАРДУ файл
   (ru ва tg) илова кун. Вагарна дар UI ба ҷои матни тарҷумашуда
   калиди хом (масалан "actions.delete") намоён мешавад
10. **Ҳар route пеш аз render месанҷад, ки кист** — дар `clientLoader`-и layout: сессия
    (корманд ё мизоҷ), permission (`ROUTE_PERMISSIONS`), дастрасии тариф (мизоҷ). Бе ҳуқуқ —
    саҳифаи возеҳ (403 / "тариф харед"), на хатогии API дар миёнаи саҳифа.
11. Ҳар `useMutation`-е, ки чизе месозад/тағйир медиҳад/нест мекунад, ки
    дар ягон **рӯйхати дигар** (query key-и дигар) низ намоён аст, бояд
    дар `onSuccess` ҳамон рӯйхат(ҳо)-ро `queryClient.invalidateQueries`
    кунад — ҳатто агар баъд аз ин фавран ба саҳифаи дигар navigate
    шавад. Хатои воқеан рухдода: `TemplatePickerModal`-и
    `automations/route.tsx` баъди сохтани flow нав рост ба
    `/automations/flows/:id` navigate мекард, БЕ `invalidateAll()` — ва
    вақте корбар бо тугмаи "Назад" ба рӯйхат бармегашт, TanStack Query
    ҳамон кэши кӯҳна (пеш аз сохтан)-ро нишон медод, то reload дастӣ.
    Худи navigate саҳви кэшро ислоҳ намекунад — invalidate бояд ҳатман
    пеш аз (ё дар баробари) navigate даъват шавад, на танҳо дар
    мутатсияҳое, ки худи саҳифаи ҷориро нав мекунанд
