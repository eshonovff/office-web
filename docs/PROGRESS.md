...
| 9 | ⚠️ **Баъд аз merge шудани ҳарду `feat/fe-inbox-block-2` ва `feat/fe-inbox-responsive` ба `dev`, ду ҷо бояд дастӣ барқарор шаванд.** 2026-08-13 4 коммити "блоки 2" (read-only composer, auto-claim, undo-send, internal note) аз бранчи адаптивӣ ҷудо ва ба бранчи нав аз `dev` кӯчонида шуданд, то мустақил санҷида шаванд. Азбаски `dev` ҳанӯз инфраструктураи адаптивиро надорад (`useInboxBreakpoint`, textarea auto-resize), ҳангоми ҷудокунӣ ду ҷо соддакарда шуд: (1) Тугмаи фиристодан дар `Composer.tsx` (item 4, ёддошти дохилӣ) дар `feat/fe-inbox-block-2` ҳамеша матн нишон медиҳад — пас аз merge бояд бозгардонида шавад ба шакли адаптивӣ: `size={breakpoint === 'mobile' ? 'icon' : 'default'}` дар тугма ва `{breakpoint !== 'mobile' && (isNoteMode ? t('sendNote') : t('send'))}` ба ҷои матни бечунучаро (ниг. коммити кӯҳна `59e8efc`, ки бо `git reset --hard` аз бранчи адаптивӣ бардошта шуд, вале то ҳол дар object database ҳаст — `git show 59e8efc -- app/routes/\(app\)/inbox/components/Composer.tsx`). (2) `MessageThread.test.tsx` дар `feat/fe-inbox-block-2` вуҷуд надорад (файл дар бранчи адаптивӣ бо коммити дигар сохта шудааст) — хатти `failureReason: null,` дар `makeMessage`-и он файл (item 3) партофта шуд, бояд бозгардонида шавад (ниг. коммити кӯҳна `f65d163`). Агар фаромӯш шаванд: TypeScript хато медиҳад (`failureReason` ҳатмист) ё UI-и мобилӣ дар режими ёддошт матни дароз нишон медиҳад | Faridun |
