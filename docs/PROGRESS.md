...
| 7 | ✅ (2026-08-12) `GET /channels/{id}` ва `GET /conversations/{id}/assignable-users` — ҳарду бо `inbox.assign`, тасдиқшуда бо curl. Навори "Ответственный" (`route.tsx`-и inbox) акнун 3 дараҷаи иерархия дорад (`getAssigneeOptions`): (1) чате кушода бошад — аъзоёни канали **ҳамон чат** (`assignable-users`), филтрро тамоман нодида мегирад; (2) чате кушода набошад, вале филтри канал интихоб шуда бошад — аъзоёни маҳз **ҳамон канал** (`GET /channels/{id}`); (3) на чат кушода, на филтр ("Все каналы") — иттиҳоди аъзоёни **ҳамаи** каналҳои дастрас (`mergeChannelMembers`), бе такрор. `PATCH .../assignedTo` бо корманди нодуруст 409 медиҳад бо сабаби фаҳмо — `client.ts`-и detail-и backend-ро аз паёми умумӣ бартарӣ медиҳад (тағйироти глобалӣ, аз санаи 12-уми август). Рӯйхат танҳо аъзоёни каналро медиҳад, на Owner/Admin (гарчанде PATCH онҳоро мегузорад) — агар UI-ро лозим ояд, ки админро низ таъин кардан мумкин бошад, backend бояд эндпойнтҳоро васеъ кунад | Faridun |
